  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-white/10 bg-[#071E34] px-4 py-4 text-white">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
        ...
        {/* Desktop nav */}
        <div className="hidden items-center gap-3 text-sm font-bold xl:flex">
          {adminLinks.map((link) => {
            ...
            return (
              <a
                key={link.href}
                href={link.href}
                className={`relative whitespace-nowrap rounded-full px-5 py-3 transition ${
                  active
        ...
        <button
          ...
          className="rounded-full border border-white/30 px-5 py-3 text-sm font-bold text-white xl:hidden"
        >
          {menuOpen ? "Close" : "☰"}
        </button>
      </div>
      ...
      {menuOpen && (
        <div className="mx-auto mt-4 max-w-7xl rounded-3xl border border-white/10 bg-[#031526] p-3 xl:hidden">
